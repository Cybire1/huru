...
async function handleStreamingChat(
  ...
  consumer: HuruConsumerRecord | null,
  requestId: string,
  ...
  const doRelease = consumer
    ...
    : () => releaseReservedCredits(project, requestId, reservedAmount);

  const doSettle = consumer
    ? (actual: number) => settleConsumerCredits(consumer, requestId, actual, reservedAmount)
    : (actual: number) => settleCredits(project, requestId, actual, reservedAmount);
  ...
  const outputStream = new ReadableStream<Uint8Array>({
    ...
    cancel() {
      if (!settled) {
        streamResult.onError();
        void doRelease();
        void failRequest(requestId, "client_disconnect", "Client disconnected.");
      }
    },
  });
...
export async function POST(request: NextRequest) {
  try { return await _POST(request); } catch (e) { console.error("[huru/chat] UNHANDLED:", typeof e, JSON.stringify(e), e instanceof Error ? e.stack : 'no stack'); throw e; }
}
...
async function _POST(request: NextRequest) {
  ...
  if (payload.stream) {
    return handleStreamingChat(request, project, consumer, requestId, estimatedCredits, {
      model: payload.model,
      messages: payload.messages,
    });
  }
  ...
  try {
    ...
    console.error("[huru/chat] runtime error:", JSON.stringify(error, Object.getOwnPropertyNames(error instanceof Error ? error : {})), error);
    await doRelease();
    await failRequest(
      requestId,
      "runtime_error",
